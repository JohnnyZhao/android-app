    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (savedInstanceState == null) {
            if (intent.getBooleanExtra(EXTRA_SHOW_PIN_SETTING, false)) {
                ...
                val auth =
                    requireNotNull(intent.getParcelableExtra<AuthorizationResponse>(EXTRA_AUTH))
                replaceFragment(
                    PermissionListFragment.newInstance(app, auth),
                    R.id.container,
                    PermissionListFragment.TAG
                )
            } else {
